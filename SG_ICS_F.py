...
while True:
    ...
    if (len(words) == 2):
        k_resiliency_ied = int(words[0])
        k_resiliency_rtu = int(words[1])        
    else:
        print ('Unmatched Input: Exit')
        sys.exit()
...
for i in range(1, num_states + 1):        
    ...
    slv.add(Sum(IExprs) > k_resiliency_ied)
...
for i in range(1, num_states + 1):        
    ...
    slv.add(Sum(IExprs) > k_resiliency_rtu)

#################################################
# Solve the formal model
status = slv.check()

f_write = open('Output_ICS_F.txt', 'a')
...
if (str(status) == 'sat'):
    ...
    f_write.write('\nIeds Corresponding to Secured states\n')
    for i in range(1, num_states + 1):
        for j in range(1, num_ieds + 1):
            print i, ' ', j, ': ', m[Secured_state_ied_relation_int[i][j]]
            f_write.write(str(i) + ' ' + str(j) + ': ' + str(m[Secured_state_ied_relation_int[i][j]]) + '\n')

    f_write.write('\nRtus Corresponding to Secured states\n')
    for i in range(1, num_states + 1):
        for j in range(num_ieds + 1, num_ieds + num_rtus + 1):
            print i, ' ', j, ': ', m[Secured_state_rtu_relation_int[i][j]]
            f_write.write(str(i) + ' ' + str(j) + ': ' + str(m[Secured_state_rtu_relation_int[i][j]]) + '\n')
