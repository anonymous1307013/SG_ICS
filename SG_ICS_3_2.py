...
UMsr_secured = [Bool('unique_msr_secured_%s' % i) for i in range(0, num_msrs + 1)]
...
for i in range(1, num_msrs + 1):
    ...
    if (len(members) > 0):
        ...
        slv.add(Implies(UMsr_secured[i], Msr_secured[members[0]]))
...
UMsr_secured_int = [Int('unique_msr_secured_int_%s' % i) for i in range(0, num_msrs + 1)]
...
f_write = open('Output_ICS_3.txt', 'w')
