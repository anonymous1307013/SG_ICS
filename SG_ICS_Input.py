...
RTU_MTU_HIERARCHY_LEVEL = 1    ## The average hierarchy level (how an rtu is connected to the MTU)

## Resiliency constraints 
MAX_UNAVAILABLE_IEDS = 3
MAX_UNAVAILABLE_RTUS = 0
...
##################################################
umsr_sets_flow = []
num_umsr_sets_flow = 0;
umsr_sets_flow.append([0])  ## First empty slot
msr_umsr = [0 for i in range(0, num_msrs + 1)]

for i in range(1, num_lines + 1):    
    if ((msr_taken[i] == 1) and (msr_taken[i + num_lines] == 1)):
        umsr_sets_flow.append([i, i + num_lines])
        num_umsr_sets_flow += 1
        msr_umsr[i] = msr_umsr[i + num_lines] = num_umsr_sets_flow

    elif (msr_taken[i] == 1):
        umsr_sets_flow.append([i])
        num_umsr_sets_flow += 1
        msr_umsr[i] = num_umsr_sets_flow

    elif (msr_taken[i + num_lines] == 1):
        umsr_sets_flow.append([i + num_lines])
        num_umsr_sets_flow += 1
        msr_umsr[i + num_lines] = num_umsr_sets_flow
        

f_write.write('# Number of power flow unique measurement sets\n')
f_write.write(str(num_umsr_sets_flow) + '\n')
f_write.write('\n')

f_write.write('# Power flow unique measurement sets\n')
for i in range(1, num_umsr_sets_flow + 1):    
    members = umsr_sets_flow[i]
    for j in range(1, len(members) + 1):
        f_write.write(str(members[j - 1]) + ' ')
    f_write.write('\n')
f_write.write('\n')

##################################################
umsr_sets_consump = []
num_umsr_sets_consump = 0;
umsr_sets_consump.append([0])  ## First empty slot

for i in range(1, num_buses + 1):    
    if (msr_taken[i + 2 * num_lines] == 1):
        members = []        
        members.append(i + 2 * num_lines)
        num_umsr_sets_consump += 1
        
        for j in range(1, num_msrs - num_buses + 1):    ## Power flow Measurements
            if (bus_msr_matrix[i][j] == 1):
                if (msr_taken[j] == 1):
                    members.append(msr_umsr[j])     ## Corresponding unique measurement set

        umsr_sets_consump.append(members)

f_write.write('# Number of power consumption unique measurement sets\n')
f_write.write(str(num_umsr_sets_consump) + '\n')
f_write.write('\n')

f_write.write('# Power consumption measurement uniqueness with respect to power flow unique measurements\n')
for i in range(1, num_umsr_sets_consump + 1):    
    members = umsr_sets_consump[i]
    for j in range(1, len(members) + 1):
        f_write.write(str(members[j - 1]) + ' ')
    f_write.write('\n')
f_write.write('\n')
